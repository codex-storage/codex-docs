...
| # (0.1.0)| Protocol | Function                                                                 | CLI option       | Example                                |
| - | -------- | ------------------------------------------------------------------------ | ---------------- | -------------------------------------- |
...
After node is up and running and port-forwarding configurations was done, we should be able to [Upload a file](./using#upload-a-file)/[Download a file](./using#download-a-file) in the network using [API](../developers/api).

You also can use [Codex App UI](https://app.codex.storage) for files upload/download.

#### (0.1.0)Codex bootstrap node
...
### (0.1.0)Run as a service in Linux
...
We can run Codex as a service via [systemd](https://systemd.io) using following steps
 ...
 2. Install Codex [using a script](https://github.com/codex-storage/get-codex) or [build from sources](./build)
    ```shell
    # (0.1.0)codex with cirdl
...
### (0.1.0)Run as a service in Windows
...
1. Create `docker-compose.yaml` file:
    ...
    services:
      codex:
        ...
        ports:
          - 8080:8080/tcp # (0.1.0)API
          - 8090:8090/udp # (0.1.0)Discovery
          - 8070:8070/tcp # (0.1.0)Transport
        volumes:
          - ./datadir:/datadir
        logging:
          driver: json-file
...
#### (0.1.0)Determine your private IP
